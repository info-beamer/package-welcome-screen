...
local background_effect = resource.create_shader[[
    ...
    void main() {
        ...
        float y1 = TexCoord.y - sinf(x*15.0 - time) * 0.02 - 0.5;
        float t1 = abs(width1 / y1);

        float y2 = TexCoord.y - sinf(x*13.0 + time) * 0.01 - 0.5;
        float t2 = abs(width2 / y2);

        float y3 = TexCoord.y - sinf(x*19.0 - time) * 0.015 - 0.5;
        float t3 = abs(width2 / y3);

        vec3 color = vec3(0.0,          min(t1,t3),     t1*3.3) +
                     vec3(min(t2,t1),   t2*5.5,         0.0) +
                     vec3(t3*8.3,       0.0,            min(t3,t2));
