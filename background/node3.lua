...
local background_effect = resource.create_shader[[
    ...
    void main() {
        ...
        float y1 = TexCoord.y - sinf(x*5.30*(TexCoord.x - time)) * 0.02 - 0.5;
        float t1 = abs(width1 / y1 / (TexCoord.x + 0.0));

        float y2 = TexCoord.y - sinf(x*9.0*(TexCoord.x + time)) * 0.05 - 0.5;
        float t2 = abs(width2 / y2 / (TexCoord.x - 0.1));

        float y3 = TexCoord.y - sinf(x*-41.90*(TexCoord.x - time)) * 0.015 - 0.5;
        float t3 = abs(width2 / y3 / (1.2 - TexCoord.x));

        vec3 color = vec3(0.0,          min(t1,t3),     t1*4.3) +
                     vec3(min(t2,t1),   t2*0.1, 	0.0) +
